...
def test_pin_states():
    ...
    rslt = [convert_pin_state(idx, pin_list) for idx in data_in]
    assert rslt == check
...
def test_system_states():
    ...
    rslt = [convert_system_state(idx, check) for idx in data_in]
    assert rslt == check
...
def test_adc_rp2_value():
    ...
    rslt = [convert_rp2_adc_value(val) for val in data_in]
    assert rslt == check
